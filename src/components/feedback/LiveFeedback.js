...
const LiveFeedback = ({ feedbacks }) => {
    return (
        <Fade bottom>
            <div>
                <h4 className='font-bold text-[#3A4374] text-md'>Live ({feedbacks?.length})</h4>
                <p className='text-sm text-[#647196]'>Released features</p>
                {feedbacks?.map((item) => (
                    <div className='bg-white  min-h-[250px] rounded-lg p-6 w-[350px] mt-8 border-t-[6px] border-[#62bcfa]'>
                        <div className='flex items-center gap-4 mb-2'>
                            <div className="w-2 h-2 rounded-full  bg-[#62BCFA]"> </div>
                            <p className='text-[#647196]'>{item.status}</p>
                        </div>
                        <Link to={`/feedback/${item.id}/detail`} className="text-[#3A4374] font-bold hover:text-[#4661e6]">{item.title}</Link>
